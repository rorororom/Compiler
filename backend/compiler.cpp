...
#define DEF_CMD(name, code, has_arg, action)                                            \
    if (source->textArray[CNTA][0] == ':')                                               \
        {                                                                               \
            UpdateLabelPosition(source, &CNTA);                                          \
        }                                                                               \
    else if (strcmp(source->textArray[CNTA], #name) == 0)                                \
    {                                                                                   \
        cmd = code;                                                                     \
        if (has_arg)                                                                    \
        {                                                                               \
            argCode = ArgCode(source, &cmd, CNTA);                                       \
            AddTwoElementsInCodeArray (source, &CNTA, cmd, argCode, outputfile);         \
        }                                                                               \
        else {                                                                          \
...
void GenerateCodeFromInput(struct Compiler *source, FILE* outputfile)
{
    ...
    int value = 0, position = 0, argCode = 0, cmd = 0, CNTA = 0;
...
int ArgCode(struct Compiler *source, int* cmd, int CNTA)
{
    assert(source != NULL);
    ...
    if ('0' <= source->textArray[CNTA + 1][0] && source->textArray[CNTA + 1][0] <= '9')
    {
        return atoi(source->textArray[CNTA + 1]);
    }
    else {
        if (source->textArray[CNTA + 1][0] == ':')
        {
            if (source->labels[source->textArray[CNTA + 1][1] - '0'] == -1)
            {
                source->unknownMarks[source->CNTAUnknownMarks++] = CNTA + 1;
                source->unknownMarks[source->CNTAUnknownMarks++] = source->position + 1;
                return -100;
            }
            else
            {
                return source->labels[source->textArray[CNTA + 1][1] - '0'];
            }
        }
        *cmd = *cmd | (1 << 5);
        return ReturnArg(source, CNTA + 1);
    }
    return value;
}
...
void UpdateLabelPosition(struct Compiler *source, int* CNTA)
{
    if (source->labels[source->textArray[*CNTA][1] - '0'] == -1)
    {
        source->labels[source->textArray[*CNTA][1] - '0'] = source->position;
    }
    (*CNTA)++;
}

void AddTwoElementsInCodeArray(struct Compiler *source, int *CNTA, int cmd, int argCode, FILE *outputfile)
{
