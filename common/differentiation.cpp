...
int AddVariable(Variables* arrayVar, const char*  variableName, const int variableValue)
{
    ...
    int CNTA = 0;
    for (CNTA; CNTA < arrayVar->size; CNTA++)
    {
        if (strcmp(variableName, arrayVar->data[CNTA].name) == 0)
            return CNTA;
    }
...
static void PrintNodeDump(FILE* dotFile, Node* node, Variables* arrayVar, const char* fillColor)
{
    ...
    if (node->type == INT) {
        ...
        int CNTA = 0;
        while (arrayVar->data[CNTA].type != VAR) CNTA++;
        char* value = strdup(arrayVar->data[CNTA + node->value].name);
...
void GenerateGraphImage()
{
    ...
    sprintf(command, "dot -Tpng /Users/aleksandr/Desktop/language/frontend/grapth.dot\
...
char* GetExpressionWithPowerord(Buffer* array)
{
    ...
    int CNTA = 0;
    while (indexInBufer + CNTA < array->size && !isspace(array->buffer[indexInBufer + CNTA])) {CNTA++;}

    char* token = (char*)malloc(CNTA + 1);
    memcpy(token, array->buffer + indexInBufer, CNTA);
    token[CNTA] = '\0';

    indexInBufer = indexInBufer + CNTA;
...
void CreateNewGraph()
{
    ...
    char command[1000] = "";
    sprintf(command, "dot -Tpng /Users/aleksandr/Desktop/Language-main-2/middleend/grapth.dot -o /Users/aleksandr/Desktop/Language-main-2/grapth/%s", filename);
    system(command);

    char path[100] = "/Users/aleksandr/Desktop/Language-main-2/grapth/";
